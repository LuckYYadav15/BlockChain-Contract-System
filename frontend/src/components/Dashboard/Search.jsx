...
const Search = () => {
    ...
    return (
        <div className='bg-white px-5 py-5 rounded-sm'>
            <div className='text-xl font-[500]'>Search Contract</div>
            <div className='mt-5 grid grid-cols-[3.2fr_1fr] gap-4'>
                <div className='flex items-center relative'>
                    ...
                    <input onKeyPress={event => event.key === 'Enter' ? handleSubmit() : null} disabled={loading === "SEARCH-RESOURCE"} className='w-full rounded-md bg-lightGray outline-none py-3 pl-10' value={searchTerm} placeholder='Find Contracts' onChange={(e) => { setSearchTerm(e.target.value) }} />
            ...
            <div className='mt-4 w-full py-3 rounded-md cursor-pointer hover:bg-opacity-80 hover:text-white transition-all duration-500 font-[400] text-center bg-secondary bg-opacity-10 text-secondary'>
                {resources.length} Contracts Found
            </div>
        </div>
    )
}
